...
class P_Robox_API
{
    ...
    public function getCurrencies($flatten = false)
    {
        ...
        if ($flatten) {
            ...
            $currencyList = $this->xPathQuery($xpath, $query, $doc->documentElement->namespaceURI);
            ...
            $query = '/*/Groups/*';
            $groups = $this->xPathQuery($xpath, $query, $doc->documentElement->namespaceURI);
            ...
            foreach ($groups as $element) {
                if ($element->nodeName == 'Group' && $element->nodeType == XML_ELEMENT_NODE) {
                    ...
                    $query = 'Items/Currency';
                    $currencyList = $this->xPathQuery($xpath, $query, $doc->documentElement->namespaceURI, $element);
                    foreach ($currencyList as $elem) {
                        if ($elem->nodeName == 'Currency' && $elem->nodeType == XML_ELEMENT_NODE) {
    ...
    public function getPaymentMethods()
    {
        ...
        $query = '/*/Methods/*';
        $methods = $this->xPathQuery($xpath, $query, $doc->documentElement->namespaceURI);
    ...
    public function getRates($outSum, $inCurrencyLabel = null)
    {
        ...
        $query = '//Currency';
        $list = $this->xPathQuery($xpath, $query, $doc->documentElement->namespaceURI);
    ...
    protected function request(array $data, $name, DOMDocument &$dom, $url = null, $sign = false)
    {
        $common_data = [
            'MerchantLogin' => $this->config->merchantid,
     ...
     * @return DOMNodeList
     */
    protected function xPathQuery(DOMXpath $xpath, $query, $ns = null, $contextNode = null)
    {
        if (!empty($ns)) {
    ...
    protected function getResultCode(DOMDocument $doc)
    {
        ...
        $query = '/*/Result/*';
        $list = $this->xPathQuery($xpath, $query, $ns);
